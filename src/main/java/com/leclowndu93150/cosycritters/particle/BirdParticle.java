...
public class BirdParticle extends TextureSheetParticle {
    ...
    private BirdParticle(ClientLevel level, double x, double y, double z, double landAtX, double landAtY, double landAtZ, SpriteSet provider) {
        super(level, x, y, z);
        this.sprite = Minecraft.getInstance().particleEngine.textureAtlas.getSprite(ResourceLocation.fromNamespaceAndPath(CosyCritters.MODID, random.nextBoolean() ? "crow_flying_left" : "crow_flying_right"));
        this.quadSize = 0;
        this.lifetime = 6000;
    ...
    public void tick() {
        ...
        if (spawnAnimation) {
            if (spawnAnimationTime != 0) {
                ...
                this.sprite = Minecraft.getInstance().particleEngine.textureAtlas.getSprite(ResourceLocation.fromNamespaceAndPath(CosyCritters.MODID, random.nextBoolean() ? "crow_left" : "crow_right"));
            }
        }
